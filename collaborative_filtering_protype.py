...
# todo: can we use wminhash somehow?
# todo: refactor for style
# todo: fix oop
# todo: cache based on hash value of input files
# todo: base class + recommender + lsh recommender (look at notebook)
class BaseCFRecommender:
    """Recommender prototype, works on small data only."""
    ...
    def _get_sim_candidates(cls, user_ratings, corr_matrix):
        ...
        try:
            sim_candidates.drop(user_ratings.index)
        except KeyError:
            pass
...
class CFRecommender(BaseCFRecommender):
    ...
    def get_recommendation(self, top_n=10):
        """Get top N recommendations knowing user ratings."""
        ...
        return list(candidates.index)[:top_n]
...
class CFLSHRecommender(BaseCFRecommender):
    ...
    def get_recommendation(self, num_results=10):
        start_time = time()

        user_profile = self._load_user_ratings(self._user_ratings_path)
        user_profile_positive = user_profile[user_profile >= 3]
        ...
        for liked_movie in liked_titles:
            ...
            idx_array = np.array(self._forest.query(m, num_results))
            if len(idx_array) == 0:
...
if __name__ == '__main__':
    # recommender = CFRecommender("../ratings_relevant_medium.csv",
    #                             "../titles.csv",
    #                             "./voting_template_filled.csv")
    #
    # #print(recommender.get_recommendation())
    #
    # recommender.generate_dataset(show_mem_usage=True)

    recommender = CFLSHRecommender("../ratings_relevant_medium.csv",
                                   "../titles.csv",
                                   "./voting_template_filled.csv",
                                   128)
    recommender.get_forest()
    for elem in recommender.get_recommendation():
        print(elem)
